...
def test_csv_index_w_header():
    index = Indxr("tests/test_data/sample_w_header.csv")
...
def test_csv_callback():
    index = Indxr(
        "tests/test_data/sample_w_header.csv", callback=lambda x: x["id"]
    )
...
def test_write_read():
    index_1 = Indxr("tests/test_data/sample_w_header.csv")
